...
def get_weapon_damage_text(w):
    if w["name"] == "Spear":
        return (str(w["damage"]) + " | Splash: " + str(w["splash_damage"]) + " / " + 
                str(w["splash_chance"]) + "%" + " | Range: " + str(w["splash_range"]))
    if w["name"] == "Dual Blades":
        return str(w["damage"]) + " | Dash: " + str(w["dash_distance"]) + " | Width: " + str(w["slash_width"])
    if w["name"] == "Boomerang":
    ...
    if w["type"] == "dash_slash":
        ...
        for monster in monsters:
            if monster["alive"] and rect_hits_line(
                monster["rect"], dash_start_pos, dash_end_pos, dash_slash_width
            ):
...
def draw_fan():
    ...
    pygame.draw.line(screen, FLAG_RED, (int(center.x), int(center.y)), (int(tip.x), int(tip.y)), 6)
    pygame.draw.circle(screen, WHITE, (int(tip.x), int(tip.y)), 5)
...
def draw_spear_in_hand():
    ...
    pygame.draw.line(screen, SPEAR_COLOR, (int(start.x), int(start.y)), (int(end.x), int(end.y)), 5)
    
    left = rotate(d, math.radians(150)) * 13
    right = rotate(d, math.radians(-150)) * 13
    pygame.draw.polygon(
        screen, SPEAR_TIP,
        [(int(end.x), int(end.y)), (int(end.x + left.x), int(end.y + left.y)), (int(end.x + right.x), int(end.y + right.y))]
    )
...
def draw_dual_blades_in_hand():
    ...
    pygame.draw.line(screen, DUAL_LONG, (int(long_start.x), int(long_start.y)), (int(long_end.x), int(long_end.y)), 5)

    short_start = center - side * 9
    short_end = short_start + d * 32
    pygame.draw.line(screen, DUAL_SHORT, (int(short_start.x), int(short_start.y)), (int(short_end.x), int(short_end.y)), 5)
...
def draw_weapon_icon():
    w = weapon()
    d = pygame.Vector2(player_direction.x, player_direction.y)
    if d.length() == 0: d = pygame.Vector2(0, 1)
    d = d.normalize()

    if w["name"] == "Sword":
        end = (player.centerx + int(player_direction.x * 35), player.centery + int(player_direction.y * 35))
        pygame.draw.line(screen, YELLOW, player.center, end, 5)

    elif w["name"] == "Gun":
        pygame.draw.rect(screen, ORANGE, (player.centerx - 8, player.centery - 8, 16, 16))
        ...
        # Draw a small purple disc indicating the boomerang is ready
        pygame.draw.circle(screen, PURPLE, (player.centerx + int(d.x*20), player.centery + int(d.y*20)), 8, 3)

    elif w["name"] == "Earthshaker":
        # Draw a hammer
        head_center = (player.centerx + int(d.x * 25), player.centery + int(d.y * 25))
...
def draw_weapon_list():
    ...
    for i, w in enumerate(weapons):
        ...
        info_text = small_font.render("Type: " + w["type"] + " | Damage: " + get_weapon_damage_text(w), True, WHITE)
...
while running:
    ...
    for spear in spears[:]:
        ...
        if hit_monster is not None:
            hit_pos = pygame.Vector2(hit_monster["rect"].centerx, hit_monster["rect"].centery)
            ...
            if splash_roll <= spear["splash_chance"]:
                ...
                for monster in monsters:
                    if monster is hit_monster: continue
                    if monster["alive"] and rect_in_circle(monster["rect"], spear_splash_pos, spear["splash_range"]):
        ...
        for monster in monsters:
            if monster["alive"] and monster not in wave["hit_targets"]:
                if rect_in_circle(monster["rect"], wave["pos"], wave["current_radius"]):
                    ...
                    m_center = pygame.Vector2(monster["rect"].centerx, monster["rect"].centery)
                    ...
                    if push_dir.length() > 0:
                        ...
                        # Clamp again after knockback so they don't fly off screen
    ...
    look = (player.centerx + int(player_direction.x * 25), player.centery + int(player_direction.y * 25))
    ...
    for monster in monsters:
        ...
        if enemy_invincible: pygame.draw.rect(screen, GREEN, rect, 3)

        hp_ratio = monster["hp"] / monster["max_hp"]
        pygame.draw.rect(screen, RED, (rect.x, rect.y - 10, 40, 5))
        pygame.draw.rect(screen, GREEN, (rect.x, rect.y - 10, max(0, hp_ratio * 40), 5))

        monster_text = small_font.render(str(monster["hp"]), True, WHITE)
        screen.blit(monster_text, (rect.x + 4, rect.y - 32))

    if dash_slashing:
        pygame.draw.line(screen, DUAL_SLASH, (int(dash_start_pos.x), int(dash_start_pos.y)), (int(dash_end_pos.x), int(dash_end_pos.y)), dash_slash_width)
        pygame.draw.line(screen, WHITE, (int(dash_start_pos.x), int(dash_start_pos.y)), (int(dash_end_pos.x), int(dash_end_pos.y)), 3)
        if show_hitbox:
            pygame.draw.circle(screen, WHITE, (int(dash_start_pos.x), int(dash_start_pos.y)), dash_slash_width // 2, 2)
            pygame.draw.circle(screen, WHITE, (int(dash_end_pos.x), int(dash_end_pos.y)), dash_slash_width // 2, 2)
    elif attacking:
        if weapon()["name"] == "Flag":
    ...
    for b in boomerangs:
        pygame.draw.circle(screen, PURPLE, (int(b["rect"].centerx), int(b["rect"].centery)), b["size"] // 2, 4)
        
    for wave in shockwaves:
        thickness = max(1, int(10 * (1 - wave["current_radius"] / wave["max_radius"])))
        pygame.draw.circle(screen, BROWN, (int(wave["pos"].x), int(wave["pos"].y)), int(wave["current_radius"]), thickness)

    if spear_splash_timer > 0 and spear_splash_pos is not None:
        pygame.draw.circle(screen, SPEAR_SPLASH, (int(spear_splash_pos.x), int(spear_splash_pos.y)), weapon()["splash_range"], 2)
        if show_hitbox:
            pygame.draw.circle(screen, WHITE, (int(spear_splash_pos.x), int(spear_splash_pos.y)), weapon()["splash_range"], 3)

    # UI / HUD
    pygame.draw.rect(screen, RED, (20, 50, 100, 10))
    pygame.draw.rect(screen, GREEN, (20, 50, max(0, player_hp), 10))

    screen.blit(font.render("Player HP: " + str(player_hp), True, WHITE), (20, 20))
    screen.blit(font.render("Weapon: " + weapon()["name"] + " | SPACE", True, weapon()["color"]), (20, 75))
    screen.blit(small_font.render("Type: " + weapon()["type"] + " | Damage: " + get_weapon_damage_text(weapon()), True, WHITE), (20, 105))
    
    screen.blit(font.render("Projectiles Active: " + str(len(bullets) + len(spears) + len(boomerangs) + len(shockwaves)), True, PURPLE), (20, 135))

    alive_monsters = sum(1 for monster in monsters if monster["alive"])
    spawn_mode_text = "STATIC" if spawn_static_monster else "CHASE"
    freeze_text = "ON" if freeze_all_monsters else "OFF"
    
    screen.blit(small_font.render("Monsters: " + str(alive_monsters) + " | N: Spawn Here | M: " + spawn_mode_text + " | B Freeze: " + freeze_text, True, MONSTER_COLOR), (20, 165))
    screen.blit(small_font.render("Player God: " + ("ON" if player_invincible else "OFF"), True, GREEN if player_invincible else GRAY), (20, 190))
    screen.blit(small_font.render("Enemy God: " + ("ON" if enemy_invincible else "OFF"), True, GREEN if enemy_invincible else GRAY), (20, 215))
    screen.blit(small_font.render("Attack Cooldown: " + str(attack_cooldown_timer), True, LIGHT_GRAY), (20, 245))

    # Weapon specific HUD rules
    if weapon()["name"] == "Spear":
        screen.blit(small_font.render("Spear: Direct " + str(weapon()["damage"]) + " | Splash Damage: " + str(last_spear_splash_damage) + " | Chance: " + str(weapon()["splash_chance"]) + "%", True, SPEAR_COLOR), (20, 280))
        screen.blit(small_font.render("Rule: Direct target does NOT take splash. Splash only hits nearby monsters.", True, LIGHT_GRAY), (20, 305))
    elif weapon()["name"] == "Dual Blades":
        screen.blit(small_font.render("Dual Blades: Dash slash in a straight line", True, DUAL_SLASH), (20, 280))
        screen.blit(small_font.render("Rule: SPACE makes player dash and slash along the dash path.", True, LIGHT_GRAY), (20, 305))
    elif weapon()["name"] == "Boomerang":
        screen.blit(small_font.render("Boomerang: Piercing projectile", True, PURPLE), (20, 280))
        screen.blit(small_font.render("Rule: Flies outwards, pauses, then chases back to the player, hitting enemies on both paths.", True, LIGHT_GRAY), (20, 305))
    elif weapon()["name"] == "Earthshaker":
        screen.blit(small_font.render("Earthshaker: Ground Slam AOE", True, BROWN), (20, 280))
        screen.blit(small_font.render("Rule: Creates an expanding shockwave that damages and forcefully knocks enemies away.", True, LIGHT_GRAY), (20, 305))

    screen.blit(font.render("Move: WASD/Arrow | TAB: Switch | I: Weapons | SPACE: Use | N: Spawn Here | M: Spawn Mode | B: Freeze Monsters", True, WHITE), (20, HEIGHT - 60))
    screen.blit(font.render("F3: Dev | F4: Player God | F5: Enemy God | F6: Hitbox | ESC: Exit", True, GRAY), (20, HEIGHT - 30))

    draw_dev()

    if weapon_list_open: draw_weapon_list()

    if player_hp <= 0:
        screen.blit(big_font.render("GAME OVER", True, RED), (WIDTH // 2 - 160, HEIGHT // 2 - 40))
